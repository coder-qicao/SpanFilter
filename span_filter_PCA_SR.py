...
import glob
from super_resolution import super_resolution_bicubic_gaussian, super_resolution_bicubic_Unsharp_Mask, \
    super_resolution_bicubic_mean
from test import metrics_images
import os

# 假设训练集中有多张彩色图像（格式可为jpg、png等）
# 训练图像（LR和对应的GT（HR）图像路径）
lr_image_path = "dataset/Set5/LRbicx2/baby.png"
hr_image_path = "dataset/Set5/GTmod12/baby.png"
# filters
F1 = super_resolution_bicubic_gaussian
F2 = super_resolution_bicubic_mean
# upscale
upscale = 3
...
# 定义要进行测试的图像列表
input_folder_path = "dataset/Set14/Set14/image_SRF_3/LR/"
input_files = os.listdir(input_folder_path)
gt_folder_path = "dataset/Set14/Set14/image_SRF_3/HR/"
...
for (new_image_path, new_target_image_path) in test_images:
    ...
    F1_R_new = F1(R_new, upscale)
    F2_R_new = F2(R_new, upscale)
    F1_G_new = F1(G_new, upscale)
    F2_G_new = F2(G_new, upscale)
    F1_B_new = F1(B_new, upscale)
    F2_B_new = F2(B_new, upscale)
